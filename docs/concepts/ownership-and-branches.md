...
from that state and deploys it as a GitHub Pages artifact. That workflow runs on
the `main` ref and reads `library` at build time; a push to `library` relays to
it. The generated site, including `catalog.json`, is not committed to the
branch.
