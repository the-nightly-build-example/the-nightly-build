...
- First-run troubleshooting: app not installed on the fork; task created
  ...
  no work available (correct silent exit); the site 404s (GitHub Pages needs a
  public repo on the free plan, or Pro; make the repo public, then
  `./setup.sh` to enable it); Actions disabled (GitHub turns workflows off on
  forks until you enable them once in the fork's Actions tab).
