# evidence: the-mechanics/random-numbers (02)

This is a second, narrow evidence request. It resolves one item the editor's
skeptic read raised: the article states as fact that HUMANS over-pick 69 when
asked for a "random" number in 1-100, and the prior record did not own that
claim. The finding: no peer-reviewed study measures a human 69 preference, and
no firsthand human source gives a verifiable per-number figure for 69. What does
exist is crowdsourced survey material. A large r/dataisbeautiful poll (n=6,750)
ranked 69 the single most-picked number in 1-100, and the Veritasium survey
(~200,000) reportedly had 69 at or near the top before its authors set the meme
numbers aside. Both are self-selected internet audiences where 69 is a known
meme, neither publishes a clean count I could open and verify, and the widely
quoted Veritasium favorites (7, 37, 73, 77) deliberately exclude 69. So the
writer can support "humans over-pick 69" only with a hedged crowdsourced survey,
not a peer-reviewed primary and not an exact rate. What is firmly established
about the 69 datapoint is the opposite direction in the model: exmergo measured
`gpt-4.1` UNDER-picking 69 (29 of 10,000, ~0.29x expected) and only hypothesizes
that 69 is a crude human favorite the model's safety training suppresses.

Everything below the new section is preserved verbatim from researcher/01. The
editor confirmed the measured LLM distributions, the mechanism, the temperature
and tool material, and the human baseline (Kubovy & Psotka; Veritasium) all
hold; nothing in this request touches them.

## The human-69 request (new in 02)

The task: find a firsthand human source measuring how often people pick 69 (or
its neighborhood) when asked for a "random" number in 1-100. Result: two
crowdsourced surveys carry a human 69 signal; no peer-reviewed study does, and no
source I could open publishes a verifiable count for 69. Recorded below with
honest classification.

```text
URL:         https://telescoper.blog/2018/04/11/what-happens-if-you-ask-people-to-pick-a-number-at-random-between-1-and-100/
Kind:        secondary — a physicist's blog (Prof. Peter Coles, then at Cardiff/Maynooth)
             reproducing and commenting on a crowdsourced r/dataisbeautiful poll. The primary is
             the Reddit poll's own dataset (n=6,750), which I could not locate or open; this blog
             owns no measurement, it reproduces the histogram and states the ranking.
Establishes: That in a documented large crowdsourced human survey, 69 was the single most-picked
             "random" number in 1-100 (then 77, then 7). This is the closest thing to a firsthand
             human source for 69 over-selection, and it points the same way the article's premise
             needs (humans over-pick 69).
Paraphrase:  6,750 people were asked to pick an integer "at random" between 1 and 100; the
             resulting histogram is far from flat, and the most popular picks were 69, then 77,
             then 7. The author frames it as the familiar "people are poor at randomness" result
             (the same conjuring-trick fact that far more people pick 7 in 1-10).
Locators:    Blog body; the embedded histogram image (x-axis to ~100, no per-number table printed).
Quote:       "The most popular picks are in fact 69, 77 and 7 (in descending order)."
Caveats:     The original r/dataisbeautiful post/URL was not found, so I could not verify a
             per-number count or percentage for 69 — only the top-3 ranking as the blog reports it.
             The sample is self-selected Reddit users, an audience where 69 is a running internet
             meme, so the over-pick is real but culturally driven and not peer-reviewed. Cite this
             as a documented crowdsourced survey, flagged as such, or cut the 69 claim; it cannot
             carry a precise rate.
```

```text
URL:         https://www.youtube.com/watch?v=d6iQrh2TK98  (see preserved source s10 below)
Kind:        secondary — crowdsourced survey reported in a video; ~200,000 respondents.
Establishes: Refinement of the human-69 question. The favorites everyone quotes from this survey are
             7, 37, 73, 77. Viewer accounts of the video (e.g. Hacker News item 40139970) say the
             raw single most-selected number was actually 69, then 42, and that the video set those
             two meme numbers aside before highlighting 37 and 73 as the "feels most random" primes.
Paraphrase:  The published/rewidely-quoted Veritasium top set (7, 37, 73, 77) deliberately excludes
             the obvious meme picks 42 and 69. So the survey plausibly does show 69 heavily
             over-picked, but the source's own featured figures do not present a 69 number, and I
             did not open the video to confirm 69's raw rank firsthand.
Locators:    Video (not opened); HN item 40139970 (viewer retelling); coverage (Upworthy, threads).
Quote:       —
Caveats:     69's rank here is a viewer's retelling, not a figure the source publishes. The clean,
             citable Veritasium favorites (7/37/73/77) do not include 69. This cannot stand as a
             firsthand human-69 figure; it corroborates the direction only.
```

Firsthand human-69 conclusion for the writer:

- There is NO peer-reviewed study measuring human over-selection of 69 in 1-100.
  Kubovy & Psotka (1976, the peer-reviewed owner of the human number-7 result)
  tested single digits, the 6-15 range, the 20s and the 70s, never 69 or 1-100.
- There IS documented large-sample crowdsourced survey evidence that humans
  over-pick 69: the r/dataisbeautiful poll (n=6,750) ranks 69 first, and the
  Veritasium survey (~200,000) reportedly has it at or near the top before the
  meme numbers are excluded. Both are self-selected internet audiences and
  neither yields a verifiable per-number figure I could open.
- Under the house citation standard (prefer the primary that owns the claim; a
  contested figure needs a primary), the human-69 claim can only be supported as
  a hedged crowdsourced survey result, not as an exact rate and not from a
  peer-reviewed source. If the writer states "humans over-pick 69," it must be
  cited to the crowdsourced survey and flagged as such; if a clean sourced figure
  is required, none exists and the honest move is to recast or cut the 69 anomaly.
- What is firmly established about the 69 datapoint remains the LLM side: exmergo
  measured `gpt-4.1` UNDER-picking 69 (29 of 10,000, ~0.29x expected) and only
  hypothesizes that 69 is a human meme-favorite suppressed by safety training.
  The human over-pick and the model under-pick are the two halves of the "runs
  the wrong way" anomaly, but only the model half rests on a verifiable firsthand
  count; the human half rests on crowdsourced survey ranking, not a peer figure.

Sources read and rejected for this request:

```text
URL: https://dejan.ai/blog/random-numbers/ — presents no firsthand human data; its human numbers are
     Veritasium's, cited not measured. The "69" count it prints (1,121/200,000) is its own AI run, not
     human data. Not a human-69 source.
URL: https://news.ycombinator.com/item?id=40139970 — a comment thread; the "69 was #1, then 42" claim
     is a viewer's retelling of the Veritasium video, no dataset or figures. Recorded above as
     corroboration of direction only, not cited as a primary.
URL: https://www.upworthy.com/pick-a-random-number-between-100-you-probably-chose-37-...  — secondary
     coverage of Veritasium; reports favorites 7/73/77/37 and does not mention 69 at all. Redundant.
URL: https://en.wikipedia.org/wiki/69_(number) — cites no survey or study of human 69 selection; only
     the number's math and meme. No empirical human-69 datapoint.
```

---

The record below is preserved verbatim from researcher/01.
