# Evidence record: the-mechanics/watermarks-in-generated-images (03)

This is a complete evidence record. It carries forward everything the round-01
and round-02 records established — round 01 remains at `researcher/01/evidence.md`
and round 02 at `researcher/02/evidence.md`, neither overwritten — and adds one
new primary read that corrects a specific claim the editor flagged: that Stable
Diffusion's training data was "never filtered" for watermarks, with "nothing
removing it" and an "absence of any filter."

**That claim is wrong, and this round's new source is the fix.** Round 02's
LAION-5B read (Appendix F.2) already found the LAION-5B paper itself does not
say whether Stable Diffusion's training subsets were watermark-filtered before
use, and that the appendix "only cites the Stable Diffusion GitHub repository
for further technical detail, which this record did not open." This round
opens that repository's own model card — Stable Diffusion's official model
documentation, written by the model's own developers — and it says plainly
that some training stages did filter on estimated watermark probability and
some did not. Three of the four released v1 checkpoints (`sd-v1-2`, `sd-v1-3`,
`sd-v1-4`) were trained wholly or in part on a subset explicitly filtered to
"an estimated watermark probability `< 0.5`." But the very first training
stage — `sd-v1-1`'s 237,000 steps on the full, un-curated `laion2B-en`, the
foundation every later checkpoint resumes from — carries no watermark filter
of any kind in the model card's own account, and neither does the following
194,000-step stage on `laion-high-resolution`, which is filtered only by
image resolution. The corrected claim is not "never filtered." It is: filtered
only in later stages, on a loose 0.5 probability threshold (looser than the
>0.8 threshold LAION itself calls conservative for its own watermark-rate
estimate), applied on top of an initial pretraining run that had no such
filter at all — which is exactly the kind of partial, leaky filter that would
reduce watermark prevalence in the output distribution without eliminating a
persistent, low-frequency ghost-watermark signal. This is detailed below under
Sources and Numbers, with the squaring-with-persistence reasoning spelled out
in a new Contradictions entry. Everything else in round 02's record — the
LAION-5B, Latent Diffusion Models, Carlini extraction, Somepalli replication,
and Getty complaint reads, and all prior Contradictions, Numbers, Source
assets, and Discarded entries — stands unchanged and is reproduced in full
below rather than merely referenced, per this round's brief.

## Sources

```text
URL:         https://github.com/CompVis/stable-diffusion/blob/main/Stable_Diffusion_v1_Model_Card.md
Kind:        primary — Stable Diffusion v1 Model Card, in the official
             CompVis/stable-diffusion GitHub repository, written by Robin
             Rombach and Patrick Esser, the model's developers (also authors
             of the Latent Diffusion Models paper already in this record).
             This is the model's own documentation of its own training
             recipe and data, not a third party's description of it. It is
             the exact document the LAION-5B paper's Appendix F.2 points to
             for training detail beyond what that paper states (Appendix
             F.2, cited in round 02's Sources: "it only cites the Stable
             Diffusion GitHub repository for further technical detail").
             Read directly from the file's raw contents (fetched from
             raw.githubusercontent.com and cross-checked word for word
             against the rendered GitHub blob page at the URL above, which
             is the document's own canonical page and where the file lives
             in the repository; the raw address was the transport, not the
             citation). A parallel copy of the same text, maintained by the
             same organization, is mirrored as the README.md on the
             `CompVis/stable-diffusion-v1-4` Hugging Face model page
             (https://huggingface.co/CompVis/stable-diffusion-v1-4);
             checked and found to state the same four checkpoints, the same
             step counts, and the same watermark-probability threshold, so
             this record cites the GitHub original rather than treating the
             mirror as a separate source.
Establishes: Exactly which of Stable Diffusion v1's four training stages used
             watermark-filtered data and which did not, the specific filter
             threshold used where one was applied, and that the filter is
             built on an estimated probability, not a certainty.
Paraphrase:  The model card documents four released checkpoints as a
             resumed sequence, each continuing from the last (Training >
             Training Procedure):
             - `sd-v1-1.ckpt`: 237,000 steps at 256x256 resolution on
               laion2B-en, then 194,000 steps at 512x512 on
               laion-high-resolution ("170M examples from LAION-5B with
               resolution >= 1024x1024"). Neither sub-stage's description
               mentions any watermark filter; laion-high-resolution's only
               stated filter is the resolution cutoff.
             - `sd-v1-2.ckpt`: resumed from sd-v1-1.ckpt. 515,000 steps at
               512x512 on "laion-aesthetics v2 5+," which the card defines
               as "a subset of laion2B-en with estimated aesthetics score
               > 5.0, and additionally filtered to images with an original
               size >= 512x512, and an estimated watermark probability
               < 0.5." The card states the watermark estimate "is from the
               LAION-5B metadata" (the same per-image classifier score
               documented in the LAION-5B paper as "the probability of the
               image containing a watermark," cited in round 02's LAION-5B
               entry) and the aesthetics score "is estimated using the
               LAION-Aesthetics Predictor V2."
             - `sd-v1-3.ckpt`: resumed from sd-v1-2.ckpt (inheriting that
               checkpoint's already-filtered training). 195,000 further
               steps at 512x512 on the same "laion-aesthetics v2 5+" subset,
               plus 10% text-conditioning dropout for classifier-free
               guidance.
             - `sd-v1-4.ckpt`: resumed from sd-v1-2.ckpt, not from
               sd-v1-3.ckpt. 225,000 further steps at 512x512 on the same
               "laion-aesthetics v2 5+" subset, plus the same 10%
               text-conditioning dropout. (v1-3 and v1-4 are sibling
               branches off v1-2, not a single further-extended chain; the
               card states both "Resumed from sd-v1-2.ckpt" separately.)
             The Limitations section (already partly used in round 02 for a
             different point) states plainly and without a watermark-specific
             carve-out: "No additional measures were used to deduplicate the
             dataset. As a result, we observe some degree of memorization for
             images that are duplicated in the training data," and separately
             that the training set "contains adult material and is not fit
             for product use without additional safety mechanisms." Neither
             limitation statement claims the watermark filter, where applied,
             is complete or guarantees watermark-free data; the filter is
             introduced only as one qualifying condition among several
             ("aesthetics score > 5.0, ... size >= 512x512, ... watermark
             probability < 0.5") for admission into one derived subset, not
             framed anywhere as a completeness guarantee over the corpus as a
             whole.
Locators:    "## Training" > "Training Data" and "Training Procedure"
             subsections (the four checkpoint bullets); "## Limitations and
             Bias" > "Limitations" (deduplication/memorization sentence,
             adult-material sentence).
Quote:       "sd-v1-1.ckpt: 237k steps at resolution 256x256 on laion2B-en.
             194k steps at resolution 512x512 on laion-high-resolution (170M
             examples from LAION-5B with resolution >= 1024x1024)."
             "sd-v1-2.ckpt: Resumed from sd-v1-1.ckpt. 515k steps at
             resolution 512x512 on laion-aesthetics v2 5+ (a subset of
             laion2B-en with estimated aesthetics score > 5.0, and
             additionally filtered to images with an original size
             >= 512x512, and an estimated watermark probability < 0.5. The
             watermark estimate is from the LAION-5B metadata, the aesthetics
             score is estimated using the LAION-Aesthetics Predictor V2)."
             "sd-v1-3.ckpt: Resumed from sd-v1-2.ckpt. 195k steps at
             resolution 512x512 on 'laion-aesthetics v2 5+' and 10% dropping
             of the text-conditioning..." "sd-v1-4.ckpt: Resumed from
             sd-v1-2.ckpt. 225k steps at resolution 512x512 on
             'laion-aesthetics v2 5+' and 10% dropping of the
             text-conditioning..." "No additional measures were used to
             deduplicate the dataset. As a result, we observe some degree of
             memorization for images that are duplicated in the training
             data." (Limitations.)
...
Paraphrase:  LAION-5B is 5.85 billion CLIP-filtered image-text pairs, split
             ...
             GitHub repository for further technical detail. This round opened
             that repository's model card (see the entry above); the LAION-5B
             paper itself still states no filtering detail beyond this
             appendix.
...
Establishes: The only figure found anywhere in this research for what share
             ...
             explicit accuracy caveat from the source itself, and the >0.8
             threshold that figure uses — now directly comparable to the
             looser <0.5 threshold the Stable Diffusion model card documents
             for its own aesthetics-filtered training subset.
...
Paraphrase:  Diffusion models are defined as "probabilistic models designed
             ...
             Appendix F.2 and the model card document as trained on
             LAION-2B-en and other LAION-5B subsets instead. The
             Limitations/societal-impact section states plainly that
             memorization was an open question at the time of writing:
             "Generative models can also reveal their training data
             [citations], which is of great concern when the data contain
             sensitive or personal information," without asserting that
             their own model does or does not do this.
...
**New this round: the model card corrects the "never filtered" claim, but
supports a more precise and still unflattering account of why the ghost
watermark persists.** The corrected claim should read: three of Stable
Diffusion v1's four checkpoints (`sd-v1-2`, `sd-v1-3`, `sd-v1-4`) were trained
wholly or in part on "laion-aesthetics v2 5+," a subset explicitly filtered to
"an estimated watermark probability `< 0.5`" — filtering did happen, and the
article should say so. But this does not make the "never filtered" claim
merely imprecise; it leaves the actual causal picture more interesting, for
three compounding reasons the model card itself establishes:

1. **The filter never touched the first, foundational stage.** `sd-v1-1`'s
   237,000-step pretraining run on the complete, unfiltered `laion2B-en` (2.32
   billion pairs, per the LAION-5B paper) carries no watermark filter in the
   model card's account, and the following 194,000-step stage on
   `laion-high-resolution` is filtered only by resolution. Every later
   checkpoint "Resumed from" this unfiltered foundation rather than
   retraining from scratch on filtered data; the watermark filter is applied
   only to what comes after, never to the base the model was first built on.
2. **Where the filter does apply, the threshold is loose relative to this
   record's own watermark-prevalence measurement.** The model card's cutoff
   is an estimated watermark probability below 0.5 — a coin-flip line, not a
   high-confidence one. LAION's own blog post, by contrast, used a stricter
   >0.8 threshold to compute its 6.1%/5.6%/4% watermark-proportion figures
   and still called those figures conservative undercounts (round 02's
   Numbers, reproduced above). A <0.5 admission threshold is not a comparable
   or stricter bar; it is a looser one, which means a meaningfully larger
   share of borderline-scored watermarked images would pass into
   "laion-aesthetics v2 5+" than would count as "watermarked" under LAION's
   own conservative statistic.
3. **The filter is a probability estimate applied once, not a guarantee
   checked against ground truth.** The model card itself calls it "an
   estimated watermark probability," sourced from "the LAION-5B metadata" —
   the same continuous classifier score the LAION-5B paper documents (round
   02's Sources) as "the probability of the image containing a watermark,"
   not a verified label. A classifier operating at a 0.5 decision boundary
   necessarily passes some fraction of true watermarked images that it scores
   just under that line, by construction, independent of how the threshold
   compares to LAION's own reporting cutoff.

Put together: the corrected claim is not that filtering was absent, but that
it was partial in scope (some stages, not the one every later checkpoint is
built on), permissive in threshold (looser than the source dataset's own
conservative watermark measurement), and probabilistic rather than exact (an
estimate, not a guarantee). That combination is fully consistent with a
persistent, low-frequency ghost-watermark signal surviving into Stable
Diffusion's output: reduced by the later-stage filter, never targeted by the
stage that established the model's base representations, and never
guaranteed to be zero even within the filtered stages. The writer should
replace "never filtered" / "nothing removing it" / "absence of any filter"
with this staged, threshold-specific account, sourced to the model card
directly rather than asserted as a blanket absence.
...
logos/posters) does not include a watermark category. Confirmed for the
...
authors treat as a ceiling; every one is stated to be a floor. The new
watermark-filter finding fits this same pattern from the other direction: the
filter that exists is looser than the measurement standard, and applied to
less than the whole training pipeline.
...
on, and no source fully reconciles them — though the model card narrows the
gap.** Carlini et al. describe the Stable Diffusion v1.4 checkpoint they
attacked as "trained on 160 million images." The LAION-5B paper's Appendix
F.2 and the model card both describe Stable Diffusion's training recipe as a
sequence of runs over LAION-2B-en (2.32 billion pairs), laion-high-resolution
(170 million), and laion-aesthetics v2 5+ (a further-aesthetics-and-watermark-
filtered subset of laion2B-en whose exact size is not stated in either
document). Somepalli et al. add a third description: Stable Diffusion v1.4
"was initially trained on over 2B images and then fine-tuned with 600M images
from the LAION Aesthetics v2 5+ subset" (Sec. 2) — the 600M figure is still
not stated in the model card itself, so it remains unreconciled with the
model card's own account, even though the model card confirms the qualitative
pipeline (initial run on the full corpus, then fine-tuning on the
aesthetics-and-watermark-filtered subset) Somepalli's sentence describes. None
of the sources state that Carlini's "160 million images" refers to a specific
named stage above; this record still could not resolve what that figure
denotes. The writer should not present a single figure for "how many images
Stable Diffusion trained on" without naming which of these it means.
...
Owner:  LAION-5B paper, Sec. 5.1; step count confirmed by the Stable
        Diffusion v1 Model Card's Training Procedure section.
Scope:  A curated subset of LAION-5B for super-resolution work, filtered
        only by original resolution (>= 1024x1024 per the model card); no
        watermark filter is stated for this subset. Used for sd-v1-1's
        second training stage (194,000 steps at 512x512).
...
Scope:  Aesthetic-score-filtered subset of LAION-5B (linear estimator on
        CLIP embeddings). "laion-aesthetics v2 5+," the specific
        further-filtered subset the Stable Diffusion v1 Model Card
        documents as the training data for checkpoints sd-v1-2 through
        sd-v1-4, is a related but distinct, more narrowly filtered subset
        whose exact size is not given in either the LAION-5B paper or the
        model card.

Figure: Stable Diffusion v1's four checkpoints and their training stages:
        sd-v1-1: 237,000 steps at 256x256 on laion2B-en, then 194,000 steps
        at 512x512 on laion-high-resolution. sd-v1-2: resumed from sd-v1-1,
        515,000 steps at 512x512 on laion-aesthetics v2 5+. sd-v1-3: resumed
        from sd-v1-2, 195,000 further steps at 512x512 on laion-aesthetics
        v2 5+ with 10% text-conditioning dropout. sd-v1-4: resumed from
        sd-v1-2 (not from sd-v1-3), 225,000 further steps at 512x512 on
        laion-aesthetics v2 5+ with 10% text-conditioning dropout.
Owner:  Stable Diffusion v1 Model Card (github.com/CompVis/stable-diffusion,
        Stable_Diffusion_v1_Model_Card.md), "Training" > "Training
        Procedure," written by the model's developers, Robin Rombach and
        Patrick Esser.
Scope:  The four checkpoints CompVis released as "Stable Diffusion v1."
        Only sd-v1-1's two stages (laion2B-en and laion-high-resolution) are
        documented as unfiltered for watermark probability; sd-v1-2,
        sd-v1-3, and sd-v1-4 are all trained wholly or in part on
        laion-aesthetics v2 5+, the watermark-filtered subset defined below.

Figure: Estimated watermark probability threshold < 0.5, applied together
        with estimated aesthetics score > 5.0 and original image size
        >= 512x512, to define "laion-aesthetics v2 5+"
Owner:  Stable Diffusion v1 Model Card, "Training" > "Training Procedure"
        (sd-v1-2.ckpt bullet); the card states the watermark estimate
        itself "is from the LAION-5B metadata," i.e. the same per-image
        classifier score the LAION-5B paper documents (Sec. 4).
Scope:  Applies only to "laion-aesthetics v2 5+," the subset used for
        sd-v1-2's 515,000-step stage and inherited by sd-v1-3's and
        sd-v1-4's further training. Does not apply to sd-v1-1's two stages.
        The threshold is on an estimated probability, not a verified label,
        and at 0.5 it is looser than the >0.8 threshold LAION's own blog
        post uses for its conservative dataset-wide watermark-proportion
        figures (below).
...
Scope:  Per named LAION-5B subset, at a >0.8 watermark-classifier
        ...
        (likely-undercounted) estimates. Not directly comparable to Stable
        Diffusion's own <0.5 training-filter threshold above as a measure of
        the same thing: one is a dataset-wide prevalence estimate at a
        strict cutoff, the other is a training-data admission threshold at a
        looser cutoff, on an overlapping but not identical population
        (laion2B-en versus laion-aesthetics v2 5+, a filtered subset of it).
...
Scope:  The specific Stable Diffusion v1.4 checkpoint the authors ran their
        ...
        with, any single subset-size figure in the LAION-5B paper's
        Appendix F.2 or the Stable Diffusion v1 Model Card's own description
        of Stable Diffusion's training recipe, or with Somepalli et al.'s
        description.
...
visual evidence here is the exhibit images themselves. The model card added
this round has no charts or figures of its own beyond one evaluation-score
plot unrelated to watermarking (the "pareto" classifier-free-guidance
comparison image), so it adds nothing new to this section.
...
URL: an AI-generated summary of arXiv:2212.03860, produced by an automated
     fetch tool during round 02's research and attributing a quote —
...
URL: https://huggingface.co/CompVis/stable-diffusion-v1-4
     — the Hugging Face model page mirroring the same model card text
     added this round. Read directly this round (both via an AI-summarizing
     fetch tool and cross-checked against the exact raw text of the GitHub
     original) and found identical in substance: the same four checkpoints,
     the same step counts, and the same "estimated watermark probability
     < 0.5" language. Not cited as a separate Sources entry because it is a
     mirror of the GitHub original rather than an independent document, and
     this record cites the GitHub repository the LAION-5B paper's Appendix
     F.2 itself points to as the canonical location.

URL: an AI-summarized first-pass read of the Stable Diffusion v1 Model
     Card (via an automated fetch-and-summarize tool, both for the GitHub
     page and the Hugging Face mirror), used only to locate the relevant
     section before the exact raw text was pulled and read directly. Not
     cited or relied upon for any figure or quote in this record; every
     figure and quote above was verified against the raw
     Stable_Diffusion_v1_Model_Card.md text and the rendered GitHub blob
     page, not the summary.
```

## Note on the previous rounds' discarded entries

Round 01 listed `arxiv.org/abs/2212.03860` (Somepalli et al.) under
Discarded, because only the abstract had been reviewed there. Round 02 read
the full paper — the CVPR 2023 camera-ready text, all 11 pages, pp. 6048-6058
— to the same standard as this record's other primaries, and the source
became a full entry in Sources rather than Discarded, where it remains above.
Carlini et al.'s own characterization of Somepalli's method (Carlini Sec. 2,
"Background") as measuring "semantic/style similarity" undersells it
somewhat: Somepalli's own formal definition (Sec. 3, quoted above) is
object-level identity, not style-or-semantic similarity in general, though
the paper does separately document pure style copying as a distinct, named
phenomenon (Sec. 7, Fig. 8).
