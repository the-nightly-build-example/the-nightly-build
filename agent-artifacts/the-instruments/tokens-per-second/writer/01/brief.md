# Writer brief 01 — the-instruments/tokens-per-second
...
- Editorial direction: `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/editorial-direction.md` (house floor, headlines
  spec, press voice, lesson identity + furniture, series prompt — the standing law).
- Commission: `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/commission.md` (this article's angle, reader, contribution,
  source obligations, prior-coverage links, structures NOT to repeat, tags).
- Voice guide: `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/writing-coach/01/voice-guide.md`.
- Evidence record: `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/researcher/01/evidence.md` — the COMPLETE set of claims
...
- Template context: `.nb-work/the-instruments/tokens-per-second/.nb-context/` (template-contract.yaml, runtime-assets.yaml,
  furniture/{engine,press,template}.md). Author against documented furniture only.

## Exact outputs (write only these)
- The article HTML at `.nb-work/the-instruments/tokens-per-second/library/the-instruments/tokens-per-second.html` (fill it in place).
- `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/writer/01/draft-handoff.md` (per the skill: original-work sentence;
  paths changed; proof result + any warnings left; evidence/voice questions).
- Any chart provenance / source asset only under `.nb-work/the-instruments/tokens-per-second/library/the-instruments/tokens-per-second/`
...
- Fill `nb-meta` with ACTUAL values: series=the-instruments, slug=tokens-per-second, template=lesson,
...
```
cd /home/user/the-nightly-build && ./nb check .nb-work/the-instruments/tokens-per-second/library/the-instruments/tokens-per-second.html --series the-instruments --repo /home/user/the-nightly-build --library /home/user/library-checkout --no-check-links
```
Then run the FINAL proof once WITH link checking (URLs must resolve) and confirm
`BLOCK: 0`:
```
cd /home/user/the-nightly-build && ./nb check .nb-work/the-instruments/tokens-per-second/library/the-instruments/tokens-per-second.html --series the-instruments --repo /home/user/the-nightly-build --library /home/user/library-checkout
```
...
`DONE writer .nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/writer/01/draft-handoff.md`, or a REQUEST line above.
