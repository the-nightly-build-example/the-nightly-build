# Researcher brief 01 — the-instruments/tokens-per-second

Load the `researcher` skill and follow it. This is invocation 01.

## Exact inputs (begin here; do not tour the repo)
- Commission: `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/commission.md` (the assignment, angle, and the required/
  starting source list — read it fully; it names every source obligation).
- Editorial direction: `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/editorial-direction.md` (house + series standards).

## Exact output (write only this)
- `.nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/researcher/01/evidence.md` — the evidence record, in the skill's
  stable sections (opening paragraph; Sources; Contradictions; Numbers; Source
  assets; Discarded).

## Source policy for this article (the-instruments)
...
- nb executable: `/home/user/the-nightly-build/nb` (run commands with the main checkout as CWD:
  ...
  `/home/user/the-nightly-build/nb history --library /home/user/library-checkout --series the-instruments --limit 20`.
...
Return exactly one line: `DONE researcher .nb-work/the-instruments/tokens-per-second/agent-artifacts/the-instruments/tokens-per-second/researcher/01/evidence.md`,
