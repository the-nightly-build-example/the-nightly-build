# Evidence: the-instruments/rouge (02)

This is the second, additive researcher round. It carries round 01 forward
unchanged in its findings and closes round 01's central gap: the
faithfulness-insufficiency evidence, recorded there only as unread leads, is now
read firsthand and pinned to its owning primaries.

The record firmly supports how a ROUGE number is built (Lin 2004, read firsthand
with the paper's own worked sentences and a second worked example computed and
hand-checked here) and the evaluation context that made it the standard (the
NIST-authored "DUC in Context"). It supports the honest center that round 01
established: ROUGE is strong on consistency but confounded, weak on coherence and
relevance (SummEval), best in its single-document-news home setting and degrading
off-domain, and structurally meaning-blind (Lin's own reversed-sentence example).
Round 01 corrected the commission's too-clean claim that meta-evaluations found
ROUGE "correlates weakly with quality and consistency," and that correction
stands.

What round 02 adds is the strongest form of the "what ROUGE cannot support"
evidence, now firsthand. Maynez et al. 2020 measured ROUGE directly against human
faithfulness and factuality judgments on abstractive summaries and found ROUGE's
Spearman correlation to be very weak (ROUGE-1/2/L between 0.10 and 0.20), far
below a textual-entailment measure (0.431 with faithfulness). Kryscinski et al.
2020 supplies the field's definitional statement that overlap metrics do not
account for factual consistency, plus the magnitude of the problem it retells
(up to 30% of abstractive summaries factually inconsistent) and an alternative
metric (FactCC). Graham 2015 supplies the older methodological critique: on
DUC-2004 the correlation of ROUGE with human assessment swings from 0.79 down to
0.29 depending purely on which of 192 variants you pick, the recommended variants
were suboptimal, and BLEU does as well as the best ROUGE.

One important synthesis holds the two faithfulness pictures together. SummEval
found ROUGE's correlation with human *consistency* moderate to strong
(ROUGE-1/2/3 = 0.53/0.59/0.71 Kendall tau) but read it as an artifact of low model
abstractiveness. Maynez, working on genuinely abstractive XSum summaries, found
ROUGE's correlation with human *faithfulness* near zero. These do not conflict:
where summaries are extractive, ROUGE tracks faithfulness by accident; where they
are abstractive, it does not track it at all. This is the defensible spine of the
commission's "cannot support" section.

Two cautions the writer must keep. First, Maynez does not show that high ROUGE
means unfaithful. Its best model on ROUGE (BERTS2S) is also its most faithful.
The finding is that ROUGE does not *track* faithfulness at the summary level, not
that the two are opposed. Second, Graham's numbers are not a "ROUGE is weak"
result. Its best variant correlates 0.786 with human assessment, a strong figure
in ROUGE's home setting. Graham is evidence of variant fragility and evaluation
methodology, not of ROUGE failing.

Source count and policy. Eight sources are now read firsthand: Lin 2004,
"DUC in Context," Bhandari 2020, Fabbri (SummEval) 2021, Maynez 2020, Kryscinski
2020, Graham 2015, and the facebook/bart-large-cnn model card. Seven are academic
primaries; the model card is a self-report artifact, not a third-party secondary.
The commission's floor of "at least 1 secondary" is the one policy item the read
set does not cleanly meet: none of the eight is an outside party reporting on
another's claim. The model card is the candidate the orchestrator may count as
the non-primary exhibit (round 01 recorded it as "one artifact," held apart from
the primaries). If the policy means a strict third-party secondary, one must still
be added. This is flagged, not padded over. See Discarded and the note below.
...
Kind:        primary artifact / self-report. Meta's own model card; it is the exhibit for the
             ...
             independent judgment that BART summarizes well. It is the record's only non-academic
             source and the candidate for the policy's non-primary slot (see the note below), but
             it is a self-report, not a third-party secondary.
...
carried unchanged from round 01; items 5-7 are new and add the faithfulness
evidence.
...
Note on the source policy's "at least 1 secondary": none of the eight read
sources is a third-party secondary. Seven are academic primaries; the BART model
card is a self-report artifact. Kryscinski 2020 acts as a secondary retelling for
one specific figure (the "up to 30%" inconsistency rate, which it attributes to
four prior works), but the paper itself is primary for its own critique and model.
If the orchestrator's reading of the policy is satisfied by a non-primary exhibit,
the model card fills that slot; if it requires a genuine outside-party secondary,
one still needs to be added. Flagged for decision, not padded.

## Numbers

Round 01 figures are preserved. New round-02 figures (Maynez, Kryscinski, Graham)
follow them.
...
Round 01 assets are preserved; new assets from Maynez and Graham follow.
...
```text
URL: Rankel et al. 2013 and Peyrard 2019 -- still unread. The brief asked for "at least one of
     Rankel 2013 or Graham 2015"; Graham 2015 was read firsthand, satisfying that requirement.
     Rankel 2013 (the "ROUGE fails to distinguish top systems" study) and Peyrard 2019 remain
     leads reached only through SummEval's and Bhandari's citations. They would strengthen the
     "ROUGE collapses among near-tied top systems" point but must be opened before being cited.
```
...
```text
URL: Kryscinski et al. 2019, "Neural Text Summarization: A Critical Evaluation" (the companion
     the brief offered as an alternative) -- not opened; the 2020 factual-consistency paper was
     read instead, per the brief's "or." It is the origin of several ROUGE/dataset-bias critiques
     retold by the 2020 paper and by SummEval, and remains an unread lead if a sharper
     ROUGE-specific critique quote is wanted.
```
