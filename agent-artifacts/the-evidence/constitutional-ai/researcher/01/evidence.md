# evidence: the-evidence/constitutional-ai (01)

Researcher model: claude-opus-4-8. Effort: high.

This record supports the commission's angle firsthand and gives the writer the
material to size it. The Constitutional AI paper (arXiv 2212.08073) reports its
two stages, its model sizes, its constitution, and its results exactly as the
commission describes: the headline claim is that one lab's 52B model, trained
with AI-generated harmlessness preferences guided by 16 written critique/revision
instructions and 16 written comparison instructions, is preferred by crowdworkers
over a human-feedback model on harmlessness while staying comparably helpful. The
"constitution" in this paper is those instruction strings, selected "in an ad hoc
manner for research purposes," not a governance charter. The evidence is thin in
three specific places the writer must respect: the paper reports Elo results as
plotted differences read off charts (Figures 2, 3, 8), not as absolute anchored
numbers; the head-to-head harmlessness win is one lab's models judged mostly by
preference comparison; and the paper's own Figure 4 finds the AI feedback labeler
only extrapolated to match human-feedback preference models *above* 52B, not at
it. Later work (Lee et al., Google, 2023) confirms RLAIF can match RLHF on three
tasks, but it does so with a plain off-the-shelf labeler prompt rather than a
written constitution, and it states outright that the 2022 paper left the
human-vs-AI-feedback question unanswered. That cuts against present-day talk of a
model "having a constitution," not against the paper's actual harmlessness result.
The evidence does not undermine the commissioned angle. It supports it, including
the sizing the commission asks for.
...
Kind:        secondary. TIME reporting (Nikita Ostrovsky and Billy Perrigo, 21 Jan
             2026) on Anthropic's revised Claude constitution. Reports on
             Anthropic from outside; does not own the underlying claim.
Establishes: How the "constitution" idea is described in the present, three years
             ...
             research method to a document that "shapes character."
Paraphrase:  Reports that Anthropic published a new version of Claude's
             constitution in January 2026, "addressed to Claude and used at
             different stages in the model's training to shape its character," and
             instructing the model to prioritize safety, ethics, Anthropic's
             guidelines, and helpfulness in that order. It contrasts the older
             approach of hand-crafted mathematical reward functions with describing
             desired behavior "in words." (A repetition, not an independent
             confirmation, of Anthropic's own framing; use it for how the idea is
             talked about now, not as proof of a mechanism.)
Locators:    Byline and dateline; body paragraphs on the constitution's role in
             training and the reward-function contrast.
Quote:       Amanda Askell (quoted by TIME): the constitution "is addressed to
             Claude and used at different stages in the model's training to shape
             its character."
```
...
verbatim principle strings already quoted above.
