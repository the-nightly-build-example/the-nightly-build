# Evidence record: the-evidence/gpt-1 (02)
...
Round 02 adds the two secondary sources the round-01 record lacked, both by
parties outside OpenAI. Sebastian Ruder's July 2018 essay (The Gradient) fixes
how the field received the recipe within weeks of release: the OpenAI Transformer,
alongside ELMo and ULMFiT, as evidence of an "ImageNet moment" for NLP built on
language-model pre-training. Liu et al.'s prompting survey (2021) fixes how the
same recipe is understood today: it names the "pre-train, fine-tune" paradigm
GPT-1 instantiates and the "pre-train, prompt, and predict" paradigm that has
displaced it. These support the commission's origin-and-present arc from outside
the authoring lab; they do not add or change any primary figure. The record now
holds eight sources, six primary and two secondary, meeting the lesson floor.
...
Kind:        secondary. Analysis by a party outside the authoring lab. Sebastian
             Ruder wrote it as a final-year PhD student at the Insight Research
             Centre and a research scientist at AYLIEN (a Dublin NLP startup), not
             at OpenAI, so he owns no GPT-1 claim and reports on the recipe from
             outside it. Published in The Gradient, 8 July 2018 — within weeks of
             GPT-1's June 2018 release. It is contemporaneous reception, not a
             retrospective, and its central prediction ("in a year's time NLP
             practitioners will download pretrained language models rather than
             pretrained word embeddings") is a forecast the field has since borne
             out, but the writer should present it as how the recipe was received
             in 2018, not as settled fact stated in 2018.
Establishes: How the field placed GPT-1's recipe the moment it landed: as one of
             three near-simultaneous results (ELMo, ULMFiT, the OpenAI Transformer)
             that together made language-model pre-training look like a paradigm
             shift for NLP, analogous to pretrained ImageNet models in computer
             vision. Ruder groups them explicitly and cites the OpenAI Transformer
             by linking the GPT-1 paper's own PDF. He frames the shift as moving
             from "just initializing the first layer of our models" (word
             embeddings) to "pretraining the entire model with hierarchical
             representations." This is outside corroboration for the commission's
             "origin of the generative-pretraining paradigm" beat: it shows the
             paradigm reading is not OpenAI's later self-description but was the
             field's own within weeks.
Paraphrase:  A leading NLP researcher, writing days after GPT-1's release, grouped
             the OpenAI Transformer with ELMo and ULMFiT as evidence that
             language-model pre-training was NLP's "ImageNet moment" — the point
             where pre-training a whole model, not just its embeddings, became the
             expected starting recipe. He predicted practitioners would soon
             download pretrained language models by default.
Locators:    Opening framing ("ImageNet moment" / same wide-ranging impact as
             pretrained ImageNet models); the "ELMo, ULMFiT, and the OpenAI
             Transformer have empirically demonstrated how language modeling can be
             used for pretraining" passage; the "why language modeling" section on
             the pretraining objective; the closing "in a year's time" prediction.
Quote:       "ELMo, ULMFiT, and the OpenAI Transformer have empirically
             demonstrated how language modeling can be used for pretraining" / "it
             is very likely that in a year's time NLP practitioners will download
             pretrained language models rather than pretrained word embeddings"
```

```text
URL:         https://arxiv.org/abs/2107.13586
Kind:        secondary. A survey and analysis by parties outside OpenAI: Pengfei
             Liu, Weizhe Yuan, Jinlan Fu, Zhengbao Jiang, Hiroaki Hayashi, Graham
             Neubig (Carnegie Mellon and collaborators), submitted 28 July 2021.
             It owns no GPT-1 claim; it analyzes the paradigm GPT-1 helped start
             from the outside, so it is context for how the recipe is understood
             today. The arXiv abstract page is the document's own page. The full
             text was read via the ar5iv mirror, but the citable address is the
             arXiv page above, where the document lives; ar5iv is only the
             transport used to read the body.
Establishes: The present-day framing of the fine-tuning-to-prompting shift, stated
             by an independent survey rather than by an OpenAI report. The survey
             names the "pre-train, fine-tune" paradigm that GPT-1 instantiates — "a
             model with a fixed architecture is pre-trained as a language model
             (LM) ... [and] then adapted to different downstream tasks by
             introducing additional parameters and fine-tuning them" — and dates a
             first "sea change" to 2017-2019 in which fully supervised, from-scratch
             training gave way to it. It then names the second shift, "as of this
             writing in 2021," in which "the 'pre-train, fine-tune' procedure is
             replaced by one in which we dub pre-train, prompt, and predict," where
             "instead of adapting pre-trained LMs to downstream tasks via objective
             engineering, downstream tasks are reformulated to look more like those
             solved during the original LM training with the help of a textual
             prompt." This corroborates, from outside the authoring lab, the
             commission's claim that GPT-1's task-specific fine-tuning is the part
             later practice moved away from, and gives the shift its standard name
             in the literature.
Paraphrase:  An independent 2021 survey describes NLP's recent history as two
             paradigm shifts: from-scratch supervised learning gave way, in
             2017-2019, to pre-training a language model and fine-tuning it per task
             (the GPT-1 recipe), and that in turn is being replaced by prompting,
             which rewrites the downstream task to look like the language-modeling
             task instead of fine-tuning the model to fit the task. The survey
             frames GPT-1's core move — supervised fine-tuning — as the step
             prompting removes.
Locators:    Abstract; Sec. 1 (Introduction), the "sea change" / "second sea
             change" paragraphs naming the "pre-train, fine-tune" and "pre-train,
             prompt, and predict" paradigms and the objective-engineering vs.
             reformulation contrast.
Quote:       "the 'pre-train, fine-tune' procedure is replaced by one in which we
             dub pre-train, prompt, and predict" / "instead of adapting pre-trained
             LMs to downstream tasks via objective engineering, downstream tasks are
...
- **The framing itself is what later practice discarded.** The paper's central
  ...
  everything the paper spends Section 3.3 building. The Liu et al. survey
  (2107.13586) names this shift firsthand from outside OpenAI: the "pre-train,
  fine-tune" paradigm GPT-1 instantiates, replaced by "pre-train, prompt, and
  predict."

- **Contemporaneous enthusiasm ran ahead of the paper's measured results.**
  Ruder's July 2018 essay (thegradient.pub/nlp-imagenet) framed the OpenAI
  Transformer as part of NLP's "ImageNet moment," a sweeping paradigm shift, at
  the same time the paper's own results were modest: gains as small as 0.6 point
  (SNLI) and 1.5 (MNLI), losses on 3 of 12 datasets, and an auxiliary objective
  that slightly lowered the average. The paradigm reading was right about the
  recipe's future and did not rest on the size of GPT-1's individual gains. A
  lesson that leans on the "ImageNet moment" reception should not let it imply the
  2018 numbers were themselves large; the secondary framing is about the method's
  reach, the paper's tables are about its immediate margins, and the two should be
  kept distinct.
...
the parameter and framing text, not a figure. The two secondary sources are prose
analysis and carry no figure this lesson should reproduce.
