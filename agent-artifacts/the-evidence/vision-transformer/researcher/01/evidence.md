# Evidence: the-evidence/vision-transformer (01)
...
that own them. Where the evidence is thin, or rather where it cuts against a naive
reading of the angle, is the present-day corrective: DeiT reaches 83.1% on ImageNet-1k
alone with no JFT, so "a Transformer needs JFT-300M" describes the original 2020
recipe, not an inherent limit of the architecture. ConvNeXt separately shows a pure CNN
matching Transformers at the same data budget. The writer must present the "you need
JFT" finding as what the ViT paper showed in 2020, not as a standing fact, and the
...
reproduce, not drop-in images.

## Discarded

```text
URL: (ViT PDF via ar5iv/arxiv html endpoints) — used only as a reading transport for the
     arxiv.org/abs pages recorded above; not recorded as a separate source, since the
     document's own page is the arxiv abstract landing.
```

No sources were read and rejected on merit. Every document opened is cited. The
Discarded section is limited to the transport note above, which the record keeps for
honesty about how the full text was read.
