# Writer brief 01 — when-ai-breaks/google-flu-trends
...
- Editorial direction: `.nb-work/when-ai-breaks/google-flu-trends/agent-artifacts/when-ai-breaks/google-flu-trends/editorial-direction.md` (house floor, headlines
  spec, press voice, lesson identity + furniture, series prompt — the standing law).
- Commission: `.nb-work/when-ai-breaks/google-flu-trends/agent-artifacts/when-ai-breaks/google-flu-trends/commission.md` (this article's angle, reader, contribution,
  source obligations, prior-coverage links, structures NOT to repeat, tags).
- Voice guide: `.nb-work/when-ai-breaks/google-flu-trends/agent-artifacts/when-ai-breaks/google-flu-trends/writing-coach/01/voice-guide.md`.
- Evidence record: `.nb-work/when-ai-breaks/google-flu-trends/agent-artifacts/when-ai-breaks/google-flu-trends/researcher/01/evidence.md` — the COMPLETE set of claims
...
- Template context: `.nb-work/when-ai-breaks/google-flu-trends/.nb-context/` (template-contract.yaml, runtime-assets.yaml,
  furniture/{engine,press,template}.md). Author against documented furniture only.

## Exact outputs (write only these)
- The article HTML at `.nb-work/when-ai-breaks/google-flu-trends/library/when-ai-breaks/google-flu-trends.html` (fill it in place).
- `.nb-work/when-ai-breaks/google-flu-trends/agent-artifacts/when-ai-breaks/google-flu-trends/writer/01/draft-handoff.md` (per the skill: original-work sentence;
  paths changed; proof result + any warnings left; evidence/voice questions).
- Any chart provenance / source asset only under `.nb-work/when-ai-breaks/google-flu-trends/library/when-ai-breaks/google-flu-trends/`
...
- Fill `nb-meta` with ACTUAL values: series=when-ai-breaks, slug=google-flu-trends, template=lesson,
...
```
cd /home/user/the-nightly-build && ./nb check .nb-work/when-ai-breaks/google-flu-trends/library/when-ai-breaks/google-flu-trends.html --series when-ai-breaks --repo /home/user/the-nightly-build --library /home/user/library-checkout --no-check-links
```
Then run the FINAL proof once WITH link checking (URLs must resolve) and confirm
`BLOCK: 0`:
```
cd /home/user/the-nightly-build && ./nb check .nb-work/when-ai-breaks/google-flu-trends/library/when-ai-breaks/google-flu-trends.html --series when-ai-breaks --repo /home/user/the-nightly-build --library /home/user/library-checkout
```
...
`DONE writer .nb-work/when-ai-breaks/google-flu-trends/agent-artifacts/when-ai-breaks/google-flu-trends/writer/01/draft-handoff.md`, or a REQUEST line above.
