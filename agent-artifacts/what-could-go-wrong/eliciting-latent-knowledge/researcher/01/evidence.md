# Evidence: what-could-go-wrong/eliciting-latent-knowledge (01)
...
binary features. The record is thin in one honest place: linear "truth-like"
directions genuinely do exist and generalize in benign, in-distribution settings
(Marks & Tegmark, 2023), and 2025 mechanistic work extracts hidden facts in
controlled toy setups. That is real and must be reported, but every such result
lives on the checkable side of the line ELK draws, not the worst-case side. The
main limitation of this record: the ELK-report and Farquhar quotes were
extracted through a fetch of the source's own page rather than read by me
character-by-character, so treat the ELK quote wording as high-confidence but
verify against the document before it carries a headline; the Burns and Marks
abstracts were captured verbatim.
...
- Internal tension worth flagging: Burns et al. claim CCS recovers knowledge
  ...
  present, which is precisely the adversarial condition ELK cares about.
...
URL: https://www.lesswrong.com/posts/bWxNPMy5MhPnQTzKz/what-discovering-latent-knowledge-did-and-did-not-find-4 — thoughtful secondary critique (Fabien Roger) but Farquhar et al. is the stronger primary for the same finding.
